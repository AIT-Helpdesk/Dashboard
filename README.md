...
- **shell** serves the sidebar shell UI, discovers page packages, and mounts each page's router at `/api/<page-id>`. The sidebar is a two-level tree -- pages sit at the top level or inside a category (e.g. "Client Info", "Ticket Info"). The whole layout (categories, their labels, and which pages are in which, in what order) is a **shared** setting, one JSON file on the server (`packages/shell/nav-layout.json`) via `GET`/`PUT /api/nav-layout` -- not per-browser `localStorage`, so everyone hitting the real dashboard URL sees the same arrangement. It's only **editable** (drag-and-drop -- drop a page onto a category's header to file it there, drop it beside another page to reorder, drop it on the thin strip below everything to pull it back out to the top level) when the app is reached via `localhost`: either a local dev copy, or RDP'ing into the production server itself and hitting its own `http://localhost:3000` directly (bypassing Caddy) to edit the *live* shared layout everyone else sees read-only via the real domain. Enforced server-side (`isLocalhostRequest()`, checked against the Host header, not the TCP peer -- which would be useless behind Caddy's reverse proxy, since every real request arrives locally-sourced at the socket level regardless of who's actually on the other end of the public domain), not just hidden in the UI. New page packages appear ungrouped at the top level on first load; a page package that's removed quietly drops out of the saved layout. Separately from that shared layout, each category is collapsible -- click its header to expand/collapse -- defaulting to collapsed; this is a per-browser viewing preference (`localStorage` key `dashboard.expandedCategories`), not part of the shared tree, so it doesn't need localhost access to change and isn't forced on other viewers. A category auto-opens if it contains the page currently being viewed (e.g. following a direct link into a nested page), and (when editable) auto-opens after a page is dropped into it, so neither case silently hides something from view. The whole sidebar can also be minimized down to a slim rail (just a toggle button, `«`/`»`) to reclaim screen width -- also a per-browser preference (`localStorage` key `dashboard.sidebarCollapsed`), same as the expanded-categories setting above, applied via a `data-sidebar-collapsed` attribute on `<html>` (not a class on `.sidebar` itself) so the same CSS rule works whether it's set by the flash-avoidance script in `index.html`'s `<head>` on load or by the toggle button's own click handler afterward.

### Deploying the sidebar layout

`packages/shell/nav-layout.json` is checked into git (by request) -- the committed file is what every deploy ends up running with, since a `git pull` overwrites it on the server just like any other tracked file. This is a deliberate tradeoff, chosen over the alternative (gitignored, deploys never touch it) specifically so the layout doesn't have to be rebuilt by hand from empty categories every time a server is freshly set up.

**The tradeoff to know**: if you edit the layout directly on the production server (RDP + its own `localhost:3000`) without also getting that change back into git, the *next* `git pull` there will silently overwrite it with whatever's still committed -- there's no merge, no warning, just whatever's in the file at deploy time. So the safe habit is: after arranging things on the server, copy `C:\apps\autotask-dashboard-git\packages\shell\nav-layout.json` back into the local repo, `git add`/`commit`/`push` it, before the next deploy happens (by you or anyone else). Editing locally first (this repo's own `localhost:3000`) and deploying that via the normal `git pull` + restart flow avoids the issue entirely, since there's nothing on the server to lose.
...
- **Shifts and Schedules** (`packages/teams-shifts`) - the General Team's Microsoft Teams Shifts schedule as a month calendar (same `gridDates`/`byDay`/day-popup convention as Service Calls) -- every shift/time-off entry's full detail (person, time, label, scheduling group, notes, activities, published vs. draft) via a click on the day. No team picker -- locked to General, resolved by name. Via Microsoft Graph's `/teams/{id}/schedule/shifts` + `/timesOff` + `/schedulingGroups` + `/timeOffReasons`, a separate app-only Graph app from CSP Customers (its own `TEAMS_SHIFTS_*` credentials, `Schedule.Read.All`/`Group.Read.All`/`User.Read.All`). Always scoped to one month at a time -- an unfiltered call returns the entire shift history back to 2020. The Graph client itself lives in `lib.js`, shared with What's On's own Team Shifts excerpt below. See its own README for other real-data quirks confirmed while building it.
- **Datto RMM** (`packages/datto-rmm`) - Total Devices, Open Alerts, and a curated shortlist of "needs attention" filters (Offline Devices, STALE Devices, Reboot Required, Antivirus Disabled, Suspended Devices, No MS Office) as donut-ring cards. Ported from a previous employee's separate app (`C:\Code\Improved-Dashboards`) -- adapted from React/ESM/fetch to this dashboard's own CommonJS/axios conventions, narrowed from that app's full 125-filter/drag-and-drop-grid design to a fixed page with a curated shortlist. See its own README for two real bugs (one in Datto's own API, one in the ported status-color logic) found and fixed while porting it.
...
3. **API permissions**: Microsoft Graph -> Delegated -> `openid`, `profile`, `email`, `User.Read`. These are typically pre-consented; no admin-consent button needed for just these four. If you're using the Voice Scheduler page (`packages/voice-scheduler`), also add `Calendars.ReadWrite` here and click **Grant admin consent for &lt;tenant&gt;** -- unlike the four above, this one is NOT pre-consented, so sign-in works either way but that page's calendar-write will fail with a scope error until this is granted.

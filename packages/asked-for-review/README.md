...
- "Completed" uses the same definition as Completed Tickets: status ID 5 (Complete) matched on `completedDate`, plus status ID 20 (Billing - Contract) matched on `resolvedDateTime` since that status never gets a `completedDate` set. Excludes issue type 14 ("Monitoring Alert").
- The selected date is only used to find its week -- the picked date itself doesn't have to be a Monday or Sunday. The week is Monday through Sunday (UTC calendar days), computed from the selected date and always shown in full regardless of where in the week the picked date falls.

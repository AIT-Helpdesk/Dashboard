...
Ingram Micro's **Cloud Marketplace API** (`api.cloud.im`) -- a wholly separate system from Autotask. Auth + pagination plumbing (`getToken()`, `fetchAllPages()`, `getSubscriptionDetail()`) lives in `@dashboard/ingram-client`, shared with the Subscriptions Expiring page (this page used to own that client privately until a second page needed it too). Credentials (`.env`): `INGRAM_CLOUD_API_BASE`, `INGRAM_CLOUD_USERNAME`, `INGRAM_CLOUD_PASSWORD`, `INGRAM_CLOUD_SUBSCRIPTION_KEY`, `INGRAM_MARKETPLACE`.

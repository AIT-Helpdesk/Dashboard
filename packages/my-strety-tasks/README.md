...
**`client.js` then RE-BUCKETS that ascending list**, by request, into Today -> Tomorrow -> Overdue -> Future -> no due date (`taskCategory()` in `render()`) -- a plain ascending sort alone puts overdue tasks (chronologically the earliest dates) BEFORE today's own, which reads wrong for a "what do I need to look at" list. A stable sort (guaranteed since ES2019) preserves each bucket's own existing date-ascending order, so Overdue still reads oldest-overdue-first and Future still reads soonest-first -- only which BUCKET comes first changes. **Team group ordering falls out of this for free**: groups are still emitted in "first task's team" order (unchanged logic), but since Today/Tomorrow tasks are now first in the re-bucketed list regardless of team, any team with a Today or Tomorrow task automatically leads, ahead of every team that only has overdue/future/no-date tasks -- no separate team-level sort needed.
...
## Due Date column -- same coloured tag as What's On's own My Strety Tasks column, and the link to Strety

The Due Date cell renders the same `.tt-tag` pill component What's On's own My Strety Tasks column uses (`ttDayTag()` in `whats-on/client.js`) -- **Today** (green), **Tomorrow** (amber), or a genuinely overdue date (due before today, red, shown as just the short date -- e.g. "3 Aug" -- no "Overdue" text label, the colour already says that). Unlike What's On's own version (which only ever shows today/tomorrow/overdue rows to begin with), this page lists EVERY open task regardless of due date -- a date further out than tomorrow gets the same pill shape with no colour at all, since it's neither today, tomorrow, nor overdue. No due date at all renders nothing. Computed client-side (`todayAestKey()`/`tomorrowAestKey()` in `client.js`), same AEST-anchoring convention every other date-scoped page on this dashboard follows, even though this page has no server-side date logic of its own to anchor to. A task due **today** is deliberately not treated as overdue -- there's still time left in the day.

**The tag itself is the link out to Strety's own web app**, by request -- NOT the title text (tried linking the title first, reverted after it looked bad in practice, in favour of matching What's On's own href-on-the-tag convention exactly). Built via `getTodoUrl()` in `@dashboard/strety-client` -- see that package's own README for the full story, but in short: a real todo's raw JSON:API response has no `links`/`permalink`/`web_url` field at all (confirmed via a live round-trip), so the URL can't be built from the API alone. The pattern (`https://2.strety.com/<WEB_ACCOUNT_ID>/todos/<todoId>`) was confirmed against a real URL a signed-in user pasted from their own browser, then independently re-confirmed: a todo in that same URL turned up in this page's own live data with the exact same generated URL. `WEB_ACCOUNT_ID` is a plain hardcoded constant (Ambient iT's own fixed Strety workspace id) -- there's no API this package found to look it up dynamically. A task whose `todoUrl` didn't resolve renders a plain (non-link) tag.

...
Dashboard page: pick a **Date** and a **Period**, see every SaaS Alerts security event of **medium or critical** severity in that window, across all customers, optionally filtered by client name and/or user name (both wildcards with `*`, independent of each other). Does not auto-load -- click Load, same convention as Completed Tickets/Ticket Times.
...
Built entirely client-side (`renderAlertSummary()`) from `data.rows` -- the exact same data the detail table renders, not a separate request -- so it's always consistent with whatever's in the table and respects both the client and user filters automatically. A client+user combination that recurs for the same alert within the period is counted once with `count > 1`, not listed as repeated rows. Verified against real data: group totals summed exactly back to the report's total count.

**Each alert-type group is individually collapsible** (`.resource-group-header--toggle`, same toggle-arrow mechanism as All Alerts below), but starts **expanded**, unlike All Alerts -- this summary already IS the at-a-glance view, so it's open by default; collapsing individual groups is for decluttering once you've seen enough of a particular alert type, not the default state. Each group's own expand/collapse state is independent of the others.
...
## Client and User name filters

Two independent fields -- Client (matched against each event's `customer.name`) and User (matched against the same resolved `user.fullName`/`user.name` label the User table column shows), either/both/neither can be set. Both use the dashboard-wide wildcard convention (`matchesWildcard()`, `@dashboard/autotask-client`) and are applied server-side, together, before the chart and report are built from the same filtered set -- so the chart, Alert Summary, and detail table all always agree with each other regardless of which filters are active. An event with no resolved user at all never matches a User filter (there's nothing to wildcard-match against), regardless of what the filter term is.

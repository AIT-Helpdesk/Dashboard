...
## Open ticket / Mark Complete popup

Clicking an entry (ticket-linked or not) opens a small popup with up to four choices, by request -- **Open ticket** (only shown when there's a linked ticket; opens it as a real popup window, same convention as every other ticket link on this dashboard), **Mark Complete**/**Mark Incomplete** (a toggle, reading the entry's own current `isComplete`), **Mark as Onsite TBA**, and **Mark as Onsite Arranged** (the two `ServiceCalls.status` values already used for this page's own left-border accent -- see "Calendar entries" above). A call with no linked ticket at all is still clickable -- Mark Complete/the two Onsite actions are the whole reason, even without a ticket to open. A middle-click or right-click -> "open in new tab" on a ticket-linked entry bypasses the popup entirely and goes straight to the ticket, same as any other real link (those never fire a plain 'click' event, which is what the popup listens for).

`PATCH /api/service-calls/:id/status` (a second, later write route, same shape as `/complete` below) sets `status` directly to `103` (Onsite Arranged) or `104` (Onsite TBA) -- Autotask's own real values for these, confirmed against the live picklist. **Not** the same values `autotask_get_field_info` itself returns for this field (`1` New, `2` Complete, `101` Canceled, `102` Canceled by Client only) -- that tool serves the exact same stale/cached 4-value picklist for this field that's already documented above as a known trap. `status` is a genuine integer field (confirmed via field info), so unlike `isComplete` (see below) no extra value-shape coercion is needed here.
...
The day popup (opened by clicking a day's number, not an individual entry) gets the same Mark Complete/Incomplete choice too, as a plain button next to each card's "Completed: Yes/No" line -- but reaches back into the OPENER window via `window.opener.toggleServiceCallComplete(id, next)` rather than duplicating the fetch/refresh logic inside that separate `document.write()`'d window (whose own `location` is `about:blank`, with no reliable base URL for a plain relative `fetch()`). Clicking it closes the day popup immediately and lets the calendar behind it refresh, rather than trying to live-update just that one card in place.

...
Clicking an entry (ticket-linked or not) opens a small popup with up to two choices, by request -- **Open ticket** (only shown when there's a linked ticket; opens it as a real popup window, same convention as every other ticket link on this dashboard) and **Mark Complete**/**Mark Incomplete** (a toggle, reading the entry's own current `isComplete`). A call with no linked ticket at all is still clickable -- Mark Complete is the whole reason, even without a ticket to open. A middle-click or right-click -> "open in new tab" on a ticket-linked entry bypasses the popup entirely and goes straight to the ticket, same as any other real link (those never fire a plain 'click' event, which is what the popup listens for).

**This is the first real WRITE this page makes against Autotask** -- everything else has been read-only until now. `PATCH /api/service-calls/:id/complete` calls `client.serviceCalls.patch(id, { id, isComplete })` -- confirmed via field info that `isComplete` is genuinely writable (not read-only/computed), so this is a plain partial update, no workflow/business-rule concerns like a ticket status change would carry. Successfully saving clears the whole `reportCacheByKey` cache (simpler and just as correct as selectively patching every month/user cache entry that might contain this call) and the client force-refetches, so the change is visible immediately rather than waiting out the 10-minute cache.

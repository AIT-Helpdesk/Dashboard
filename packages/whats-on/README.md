...
- **My Strety Tasks** additionally surfaces the signed-in user's own **overdue** to-dos (`due_date` before today, not just today/tomorrow) -- deliberately different scope from the other two columns, which stay strictly today/tomorrow, since a personal to-do list is exactly the kind of thing where "it's overdue" is more useful to surface than to hide. Overdue rows sort after the today/tomorrow ones (oldest-due first within that group -- see `isOverdue()`/sort comment in `server.js`) and render with a red tag showing just the due date instead of "Today"/"Tomorrow" (no "Overdue" text label -- the red already says that; see `ttDayTag()` in `client.js`), so a glance at the list still says *when* each one was due. Because this can legitimately run to dozens of rows for someone who's fallen behind, `.tt-list` caps its own height and scrolls internally (`packages/shell/public/styles.css`) rather than stretching the whole three-column row to match it.

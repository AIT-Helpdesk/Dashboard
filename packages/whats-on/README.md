...
- **Service Calls** rows are tagged with a link to the call's Autotask ticket where one exists (resolved the same way Service Calls' own report resolves `ticketUrl`, via the joined ServiceCallTicket -- see that page's README) -- clicking the tag opens the ticket as a real popup window (explicit `window.open(..., 'width=1200,height=900')`), same convention every other ticket link on this dashboard uses, not just a new tab. Calls with no linked ticket keep the plain (non-link) tag.

...
- **Service Calls** rows are tagged with a link to the call's Autotask ticket where one exists (resolved the same way Service Calls' own report resolves `ticketUrl`, via the joined ServiceCallTicket -- see that page's README) -- clicking the Today/Tomorrow tag opens the ticket in a new tab, same pattern as other cross-links on this dashboard. Calls with no linked ticket keep the plain (non-link) tag.
- **My Strety Tasks** additionally surfaces the signed-in user's own **overdue** to-dos (`due_date` before today, not just today/tomorrow) -- deliberately different scope from the other two columns, which stay strictly today/tomorrow, since a personal to-do list is exactly the kind of thing where "it's overdue" is more useful to surface than to hide. Overdue rows sort after the today/tomorrow ones (oldest-due first within that group -- see `isOverdue()`/sort comment in `server.js`) and render with a red "Overdue -- <date>" tag instead of "Today"/"Tomorrow", so a glance at the list still says *when* each one was due. Because this can legitimately run to dozens of rows for someone who's fallen behind, `.tt-list` caps its own height and scrolls internally (`packages/shell/public/styles.css`) rather than stretching the whole three-column row to match it.

...
- **Priority** (`priority`, the "magnet", labelled "Workshop Status" in the legend under the table) -- `urgent` (red) / `complete` (green) / `nearly_complete` (blue) / `in_progress` (orange) / `next_up` (purple) / `not_started` (grey, no background tint -- see the CSS comment on `.wsp-row-tint--*`). Defaults to `not_started`. Shown as a dot leading the Status cell (its own click target -- see below) plus a full label in the legend under the table. Deliberately a manually-chosen field, not computed from a ticket's due date -- plenty of jobs have no linked ticket at all. Originally a time-urgency scale (Today/Tomorrow/2-4 days/Over 4 days); retiered to this completion-progress scale by request -- see `migrateRetierPriorityToStatusStyle` in `db.js`.
- **Status** (`workflow_stage`) -- `new` / `free_text` / `in_car` / `ready_to_ship` / `ready_for_pickup` / `sent` / `delivered` / `collected`. Purely informational workflow tracking -- reaching `collected` does **not** complete/archive the job on its own, that's the separate Mark Complete tick. `free_text` and `in_car` both pair with a companion `workflow_stage_text` column (type anything / whose car it's in), shown as an inline text field once picked. `ready_for_pickup`/`sent`/`delivered`/`collected` render in green in the list (reads as "done and on its way out/gone").

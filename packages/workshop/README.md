...
- **Status** (`workflow_stage`) -- `new` / `free_text` / `in_car` / `ready_to_ship` / `ready_for_pickup` / `sent` / `delivered` / `collected`. Purely informational workflow tracking -- reaching `collected` does **not** complete/archive the job on its own, that's the separate Mark Complete tick. `free_text` and `in_car` both pair with a companion `workflow_stage_text` column (type anything / whose car it's in), shown as an inline text field once picked. `ready_for_pickup`/`sent`/`delivered`/`collected` render in green in the list (reads as "done and on its way out/gone"); `new`/`ready_to_ship`/`in_car` render in red (reads as "not yet actioned/still waiting on the workshop").
